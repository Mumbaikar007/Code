...
for prods in theProds:
    ...
    if currentProd[2] in nonTerminals:
        if currentProd[2] not in nonTerminalsAdded:
            ...
            [theProds.append(k) if givenProductions[k][0] == currentProd[2] else None for k in range (len(givenProductions) )]
sorted(theProds)
        ...
        if b < len(givenProductions[a]):
            if givenProductions[a][b] not in jumpsMade:
                ...
                for j in range(i, len(s1.startOfProductions)):
                    ...
                    if d < len(givenProductions[c]) and givenProductions[a][b] == givenProductions[c][d]:
...
for i in range(len(allStates)):
    ...
    if len(allStates[i].toNextState) == 0 and i != 1:
        for j in range(1,len(terminals)+1):
            a[i][j] = 'r' + str(allStates[i].statesProductions[0])  #somewhere here for adding r to a[i][j]s
